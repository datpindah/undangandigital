...
serve(async (req: Request) => {
  ...
  // Path: /invitations, /invitations/my, /invitations/:slug, /invitations/:id/couple-photo, /invitations/:id/music
  const parts = url.pathname.replace(/^\/invitations\/?/, "").split("/").filter(Boolean);
  // parts[0] = 'my' | slug | id
  // parts[1] = 'couple-photo' | 'music' (optional)
  ...
  try {
    ...
    if (req.method === "POST" && parts.length === 0) {
      ...
      const finalSlug = slug
        ...
        : `${groom_name}-${bride_name}-${Date.now()}`.toLowerCase().replace(/\s+/g, "-");
    ...
    // -------------------------------------------------------
    // GET /invitations/my — list own invitations
    // -------------------------------------------------------
    ...
    // -------------------------------------------------------
    // GET /invitations/:slug — public view
    // -------------------------------------------------------
    ...
    if (req.method === "PUT" && parts.length === 1) {
      ...
      // Whitelist updatable fields (fix SQL injection from original code)
    ...
    if (req.method === "DELETE" && parts.length === 1) {
      ...
      // Get gallery images and couple photos for storage cleanup
      ...
      const extractPath = (url: string | null) => {
        if (!url) return;
        // path stored as full URL or just path
      ...
      // Delete invitation (cascades to guests, rsvps, gallery)
      const { error } = await supabase
        .from("invitations")
        .delete()
        .eq("id", id)
    ...
    if (req.method === "POST" && parts[1] === "couple-photo") {
      ...
      const filePath = `${user.id}/${id}/${role}-${Date.now()}.${ext}`;
      ...
      await supabase.from("invitations").update({ [column]: publicUrl.publicUrl }).eq("id", id);
    ...
    if (req.method === "POST" && parts[1] === "music") {
      ...
      const filePath = `${user.id}/${id}/music-${Date.now()}.${ext}`;
      const arrayBuffer = await file.arrayBuffer();
      ...
      await supabase.from("invitations").update({ music_url: publicUrl.publicUrl }).eq("id", id);
